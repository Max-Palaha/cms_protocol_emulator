...
def run_sia_server():
    ...
    try:
        ...
        if not is_port_open(HOST, PORT):
            print("\n[ERROR] Port is not open. Process output:")
            print("\n".join(output))
...
async def test_nak_response(run_sia_server):
    ...
    assert "_20" in response  # timestamp present
...
async def test_nak_2_then_ack(run_sia_server):
    ...
    r3 = await send_event_and_receive_response(event)
...
async def test_time_custom_response(run_sia_server):
    ...
    # Replay the event with incremented sequence
    event2 = b'4AA9003C"BR"0001R0L0A0#acct[]\r'
